...
lzw_decompress("\0\0\0` \0„\0\n @\0´C„è\"\0`EãQ¸àÿ‡?ÀtvM'”JdÁd\\Œb0\0Ä\"™ÀfÓˆ¤îs5›ÏçÑAXPaJ“0„¥‘8„#RŠT©‘z`ˆ#.©ÇcíXÃşÈ€?À-\0¡Im? .«M¶€\0È¯(Ì‰ıÀ/(%Œ\0");}elseif($_GET["file"]=="functions.js"){header("Content-Type: text/javascript; charset=utf-8");echo
...
html_select($C,$Ye,$Y="",$Re=true,$Qd=""){if($Re)return"<select class='form-control' name='".h($C)."'".(is_string($Re)?' onchange="'.h($Re).'"':"").($Qd?" aria-labelledby='$Qd'":"").">".optionlist($Ye,$Y)."</select>";$J="";foreach($Ye
...
get_password(){$J=$GLOBALS['password'];if(is_array($J))$J=($_COOKIE["adminer_key"]?decrypt_string($J[0],$_COOKIE["adminer_key"]):false);return$J;}function
...
auth_url($ii,$N,$V,$m=null){global$Wb;preg_match('~([^?]*)\\??(.*)~',remove_from_uri(implode("|",array_keys($Wb))."|username|".($m!==null?"db|":"").session_name()),$B);return base_url()."admin/database/phpmyadmin?".(sid()?SID."&":"").($ii!="server"||$N!=""?urlencode($ii)."=".urlencode($N)."&":"")."username=".urlencode($V).($m!=""?"&db=".urlencode($m):"").($B[2]?"&$B[2]":"");}function
is_ajax(){return($_SERVER["HTTP_X_REQUESTED_WITH"]=="XMLHttpRequest");}function
redirects($A,$pe=null){if($pe!==null){restart_session();$_SESSION["messages"][preg_replace('~^[^?]*~','',($A!==null?$A:$_SERVER["REQUEST_URI"]))][]=$pe;}if($A!==null){if($A=="")$A=".";header("Location: $A");exit;}}function
...
false;}if($Xf)redirects($A,$pe.$Qg);return
true;}function
...
name(){return"<a href='.'>&laquo; back</a>";}function
credentials(){return
...
head(){return
true;}function
loginForm(){global$Wb;echo $title,'<table cellspacing="0">
<tr><th>System<td>',html_select("auth[driver]",$Wb,DRIVER),'<tr><th>Server<td><input class="form-control" name="auth[server]" value="'.base64_decode($GLOBALS['server']).'" title="hostname[:port]" placeholder="localhost" autocapitalize="off">
<tr><th>Username<td><input class="form-control" name="auth[username]" id="username" value="',base64_decode($GLOBALS['user']),'" autocapitalize="off">
<tr><th>Password<td><input class="form-control" type="password" name="auth[password]" value="',base64_decode($GLOBALS['password']),'">
<tr><th>Database<td><input class="form-control" name="auth[db]" value="',base64_decode($GLOBALS['database']),'" autocapitalize="off">
</table>
...
tableStructurePrint($p){echo"<table class='table table-responsive table-hover' cellspacing='0'>\n","<thead><tr><th>".'Column'."<td>".'Type'.(support("comment")?"<td>".'Comment':"")."</thead>\n";foreach($p
...
<p id="dbs">
';hidden_fields_get();$Gb=" onmousedown='dbMouseDown(event, this);' class='form-control' onchange='dbChange(this);'";echo"<span title='".'database'."'>DB</span>: ".($l?"<select name='db'$Gb>".optionlist(array(""=>"")+$l,DB)."</select>":'<input name="db" value="'.h(DB).'" autocapitalize="off">'),"<input type='submit' value='".'Use'."'".($l?" class='hidden'":"").">\n";if($xe!="db"&&DB!=""&&$g->select_db(DB)){if(support("scheme")){echo"<br>".'Schema'.": <select name='ns'$Gb>".optionlist(array(""=>"")+$b->schemas(),$_GET["ns"])."</select>";if($_GET["ns"]!="")set_schema($_GET["ns"]);}}echo(isset($_GET["sql"])?'<input type="hidden" name="sql" value="">':(isset($_GET["schema"])?'<input type="hidden" name="schema" value="">':(isset($_GET["dump"])?'<input type="hidden" name="dump" value="">':(isset($_GET["privileges"])?'<input type="hidden" name="privileges" value="">':"")))),"</p></form>\n";}function
tablesPrint($T){echo"<ul id='tables' onmouseover='menuOver(this, event);' onmouseout='menuOut(this);'>\n";foreach($T
as$R=>$P){echo'<li><a title="Show data" href="'.h(ME).'select='.urlencode($R).'"'.bold($_GET["select"]==$R||$_GET["edit"]==$R,"select").">".'select'."</a> ";$C=$this->tableName($P);echo(support("table")||support("indexes")?'<a href="'.h(ME).'table='.urlencode($R).'"'.bold(in_array($R,array($_GET["table"],$_GET["create"],$_GET["indexes"],$_GET["foreign"],$_GET["trigger"])),(is_view($P)?"view":"structure"))." title='".'Show structure'."'>$C</a>":"<span>$C</span>")."\n";}echo"</ul>\n";}}$b=(function_exists('adminer_object')?adminer_object():new
Adminer);if($b->operators===null)$b->operators=$Ve;function
page_header($vh,$n="",$Va=array(),$wh=""){global$ca,$ia,$b,$Wb,$x;page_headers();if(is_ajax()&&$n){page_messages($n);exit;}$xh=$vh.($wh!=""?": $wh":"");$yh=strip_tags($xh.(SERVER!=""&&SERVER!="localhost"?h(" - ".SERVER):"")." - ".$b->name());
echo '<!DOCTYPE html>
...
<link rel="stylesheet" type="text/css" href="'.base_url().'admin_assets/bootstrap/css/bootstrap-cosmo.min.css"/>
<link rel="stylesheet" type="text/css" href="'.base_url().'admin_assets/dist/css/adminer.css"/>
<script type="text/javascript" src="'.base_url().'admin_assets/js/adminer.js"></script>
 <link rel="icon" type="image/x-icon" href="'.base_url().'admin_assets/dist/img/gear.ico">
...
';echo'<div id="menu">
';$b->navigation($xe);if($xe!="auth"){echo'<form action="" method="post">
<p class="logout" style="margin-left:8px">
...
<table cellspacing="0" class="table table-responsive table-hover">
...
<table class="table table-responsive table-hover" cellspacing="0" id="partition-table"',($sf?"":" class='hidden'"),'>
...
<form action="" method="post">
<table cellspacing="0" class="nowrap table table-responsive table-hover">
<thead><tr>
...
<form action="" method="post">
<table cellspacing="0" onclick="tableClick(event);" ondblclick="tableClick(event, true);" class="nowrap checkable table table-responsive table-hover">
